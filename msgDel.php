...
if($text && $encrypted != "")
{
	...
	{	
		...
		$str_edit = $data_send_mas['str_edit'];	
		...
		if($sid_b == $sid && $user_id > 0 && $packet_key != "" && $str_edit != "")
		{			
		
			$del_msgs_str = '';
			
			$data_edit_mas = explode(',', $str_edit);
			
			$count_data_edit_mas = count($data_edit_mas);
			
			for($k = 0; $k < $count_data_edit_mas; $k++)
			{	
				$msg_key = $data_edit_mas[$k];
			
				if($msg_key != "")
				{
					if(getMsgEditAccess($msg_key, $user_id))
					{
						$del_id = delMsg($msg_key, $user_id);
						
						if($del_id > 0)
							$del_msgs_str .= $msg_key . ',';
			...
			$data_mas['del_msgs_str'] = $del_msgs_str;
